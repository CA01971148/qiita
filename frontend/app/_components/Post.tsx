...
function Post() {
  ...
  useEffect(() => {
    const parseMarkdown = async () => {
      try {
        const htmlContent = await marked(content); // Promiseとして解決する場合
        const sanitizedContent = DOMPurify.sanitize(htmlContent); // サニタイズ
        setPreviewContent(sanitizedContent); // プレビュー用のstateを更新
      } catch (error) {
    ...
    if (content) {
      parseMarkdown(); // contentが存在する場合のみ実行
    } else {
      setPreviewContent(''); // contentが空ならプレビューも空に
    }
  }, [content]);
